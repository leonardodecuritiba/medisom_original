(function ($) {
    $.fn.jqueryTree = function (options) {
        ...
        return this.each(function () {
            var jqueryTree = $(this).data('jqueryTree');
            if (!jqueryTree) {
                jqueryTree = new Bonsai(this, options);
                $(this).data('jqueryTree', jqueryTree);
            }
            if (typeof options == 'string') {
                var method = options;
                return jqueryTree[method].apply(jqueryTree, [].slice.call(args, 1));
            }
        });
    };
    $.jqueryTree = {};
    ...
    var Bonsai = function (el, options) {
        var self = this;
        options = options || {};
        this.options = $.extend({}, $.jqueryTree.defaults, options);
        this.el = $(el).addClass('jqueryTree').data('jqueryTree', this);

        this.guid = Math.round(Math.random() * 1e8);
        this.generatedIdPrefix = 'jqueryTree-generated-' + this.guid + '-';
        this.specifiedIdPrefix = 'jqueryTree-specified-' + this.guid + '-';
        ...
        if (this.isRootNode()) {
            ...
            this.el.on('click', '.jqueryTreeThumb', function (ev) {
    ...
    Bonsai.prototype = {
        ...
        setExpanded: function (listItem, expanded) {
            ...
            if (expanded) {
                ...
                $(listItem.data('subList')).css('display', 'block');
            }
            else {
                listItem = $(listItem).addClass('collapsed')
                    .removeClass('expanded');
                $(listItem.data('subList')).css('display', 'none');
            }
        },
        expandAll: function () {
        ...
        update: function () {
            ...
            this.el.children().each(function () {
                ...
                // insert a jqueryTreeThumb if it doesn't already exist
                if (item.children().filter('.jqueryTreeThumb').length == 0) {
                    var jqueryTreeThumb = $('<div class="jqueryTreeThumb"></div>');
                    item.prepend(jqueryTreeThumb);
                }
                ...
                subLists.each(function () {
                    ...
                    var exists = !!$(this).data('jqueryTree');
                    $(this).jqueryTree(exists ? 'update' : self.options);
                });
            });
        ...
        insertCheckbox: function (listItem) {
            ...
            var id = this.checkboxId(listItem),
                checkbox = $('<input type="checkbox" name="permission[]" id="' + id + '" /> '
                ),
        ...
        liId: function (listItem) {
            ...
            var dataId = listItem.attr('data-jqueryTree-id');
